...
import magmaout.mappet.api.scripts.code.mappet.MappetImage;
import magmaout.mappet.api.scripts.code.mappet.MappetUIBuilder;
import magmaout.mappet.api.scripts.user.blocks.IScriptBlockState;
...
import magmaout.mappet.api.scripts.code.blocks.ScriptBlockState;
...
public class ScriptFactory implements IScriptFactory {
    ...
    public int parseKey(String name) {
        Class<? extends Object> clazz = Keyboard.class;
        int key = -1;
        try {
            Field field = clazz.getDeclaredField("KEY_" + name.toUpperCase());
            field.setAccessible(true);
            key = (Integer) field.get(Keyboard.class);
    ...
    @Override
    public IMappetImage createImage(String path) {
