...
import magmaout.mappet.api.scripts.code.ScriptRayTrace;
import magmaout.mappet.api.scripts.code.ScriptWorld;
import magmaout.mappet.api.scripts.code.entities.ai.EntitiesAIPatrol;
import magmaout.mappet.api.scripts.code.entities.ai.EntityAILookAtTarget;
...
import magmaout.mappet.api.scripts.code.mappet.MappetStates;
...
import magmaout.mappet.api.scripts.user.data.ScriptBox;
...
import magmaout.mappet.api.scripts.user.mappet.IMappetStates;
...
import magmaout.mappet.network.scripts.PacketWorldMorph;
...
public class ScriptEntity<T extends Entity> implements IScriptEntity {
    ...
    public float getWalkedDistance() {
        return this.entity.distanceWalkedModified;
    }
    ...
    public ScriptVector getRotations() {
        float yawHead = this.getYawHead();
        if (this.isPlayer())
            yawHead = ((EntityPlayer) this.entity).renderYawOffset;
        return new ScriptVector(this.getPitch(), this.getYaw(), yawHead);
    }
    ...
    public void setRotations(float pitch, float yaw, float yawHead) {
        ...
        if (!this.isPlayer()) {
            ...
            for (EntityPlayer player : tracker.getTrackingPlayers(this.entity)) {
                Dispatcher.sendTo(new PacketEntityRotations(this.entity.getEntityId(), pitch, yaw, yawHead), (EntityPlayerMP) player);
            }
        }
    }
    ...
    public float getYawHead() {
        if (this.isPlayer()) {
            return ((EntityPlayer) this.entity).renderYawOffset % 360;
        } else {
            return this.entity.getRotationYawHead() % 360;
        }
    }
    ...
    @Override
    public void lockRotations(float pitch, float yaw, float yawHead) {
        if (this.isPlayer()) {
            EntityPlayerMP player = (EntityPlayerMP) this.entity;
            Location location = Location.get(player);

            location.setPitchClamping(pitch, pitch);
            location.setYawClamping(yaw, yaw);

            Dispatcher.sendTo(new PacketCapability(location.serializeNBT(), CapabilityTypes.LOCATION), player);
        } else {
            this.entity.getEntityData().setBoolean("rotationsLocked", true);
    ...
    public void unlockRotations() {
        if (this.isPlayer()) {
            ...
            this.entity.getEntityData().setBoolean("rotationsLocked", false);
        }
    }

    @Override
    public boolean isRotationsLocked() {
        if (this.isPlayer()) {
            Location location = Location.get((EntityPlayerMP) this.entity);
            return location.pitchClamping.get(0) == location.pitchClamping.get(1) && location.yawClamping.get(0) == location.yawClamping.get(1);
        } else {
            return this.entity.getEntityData().getBoolean("rotationsLocked");
        }
    }

    @Override
    public void rotateTo(String interpolation, int duration, float pitch, float yaw, float yawHead) {
        if (this.isPlayer()) {
            EntityPlayerMP player = (EntityPlayerMP) this.entity;
            Location location = Location.get(player);

            location.setStart(player.rotationPitch, player.rotationYaw, player.renderYawOffset);
            location.setEnd(pitch, yaw, yawHead);
            location.setDuration(duration * 50L);
            location.setInterp(interpolation);

            Dispatcher.sendTo(new PacketCapability(location.serializeNBT(), CapabilityTypes.LOCATION), player);
        } else {
            this.to(interpolation, duration, pitch, yaw, yawHead, 1);
        }
    }
    ...
    public void moveTo(String interpolation, int duration, double x, double y, double z) {
        if (this.isPlayer()) {
            this.to(interpolation, duration, x, y, z, 0);
        } else {
            this.setAIEnabled(false);
            this.to(interpolation, duration, x, y, z, 0);
            CommonProxy.eventHandler.addExecutable(new RunnableExecutionFork(duration, () -> this.setAIEnabled(true)));
        }
    }

    private void to(String interpolation, int duration, double x, double y, double z, int type) {
        Interpolation interp = Interpolation.valueOf(interpolation.toUpperCase());

        for (int i = 0; i < duration; i++) {
            double progress = (double) i / (double) duration;
            ScriptVector vec = new ScriptVector(
                interp.interpolate(this.entity.posX, x, progress),
                interp.interpolate(this.entity.posY, y, progress),
                interp.interpolate(this.entity.posZ, z, progress)
            );
            if (type == 0) {
                CommonProxy.eventHandler.addExecutable(new RunnableExecutionFork(i, () -> this.setPosition(vec.x, vec.y, vec.z)));
            } else {
                CommonProxy.eventHandler.addExecutable(new RunnableExecutionFork(i, () -> this.setPosition(vec.x, vec.y, vec.z)));
            }
        }
