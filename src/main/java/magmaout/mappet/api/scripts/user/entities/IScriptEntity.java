...
import magmaout.mappet.api.scripts.user.IScriptFactory;
...
public interface IScriptEntity {
    ...
    /**
     * Get entity walked distance.
     *
     * <pre>{@code
     *    var distance = c.getSubject().getWalkedDistance();
     *
     *    c.send(c.getSubject().getName() + " walked " + distance);
     * }</pre>
     */
    public float getWalkedDistance();

    /**
     * Get entity life time.
     *
     * <pre>{@code
     *    var time = c.getSubject().getLifeTime();
     *
     *    c.send(c.getSubject().getName() + " lived " + time);
     * }</pre>
     */
     ...
     * s.lockRotations(rot.y, rot.x, rot.z);
     * }</pre>
     *
     ...
     *
     * <pre>{@code
     *    if (c.getSubject().isRotationsLocked())
     *    {
     *        // Do something
     *    }
     * }</pre>
    ...
    /**
     * <p>Rotate the entity to the specified rotation (<b>pitch, yaw, yawHead</b>)
     ...
     *    var s = c.getSubject();
     *    var pos = s.getPosition();
     *    s.rotateTo("quad_out", 30, s.pitch, s.yaw + 90, s.yawHead + 90);
     * }</pre>
     *
     * @param interpolation The interpolation type used for the movement.
     * @param duration The duration of the movement in ticks.
     * @param pitch             The target pitch-rotation for the entity.
     * @param yaw             The target yaw-rotation for the entity.
     * @param yawHead             The target yawHead-rotation for the entity.
     */
